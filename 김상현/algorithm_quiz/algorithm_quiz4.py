...
def greedy():
    total_money = int(input("액수 입력 : "))
    kinds_of_money = input("동전의 종류 : ").split(" ")
    need_quantity = []
    text = []
    for i in range(len(kinds_of_money)):
        need_quantity.append(total_money // int(kinds_of_money[i]))
        total_money %= int(kinds_of_money[i])
        text.append("{}원 동전 {}개".format(kinds_of_money[i], need_quantity[i]))
    return ', '.join(text)
